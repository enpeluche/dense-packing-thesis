...
\usepackage{hyperref} %must be last package to be imported
...
\begin{figure}
    \includegraphics[width=8cm,height=5cm]{logo_ubfc.png}\\\\\\\\\\
\end{figure}
\begin{center}
    {\fontsize{20}{20}\selectfont Université Bourgogne-Franche-Comté\\}
    {\fontsize{15}{15}\selectfont UFR - Sciences et techniques }\\
    {\fontsize{15}{15}\selectfont 9 Avenue Alain Savary, Dijon }\\
    {\fontsize{15}{15}\selectfont Année universitaire 2021-2022 }
    \\\
    \\\
    \\\
...
\begin{center}
\textbf{\\\\ \Huge Empilement compact et inégalité isopérimétrique} 
...
\begin{center}
    {\fontsize{15}{15}\selectfont Projet de Licence 3 mathématique }\\
    {\fontsize{15}{15}\selectfont Présenté par Noirot Lucas et Routaboule Régis}
\end{center}
...
L'empilement compact, qu'est ce que c'est ? 
...
On peut par exemple parler d'empilement d'orange, de ballons $\cdots$\\
L'empilement compact fascine depuis longtemps les mathématiciens. En 1606, l'assistant de Walter Raleigh se pose le problème de l'empilement des boulets de canons. Ils vont alors correspondre avec Johannes Kepler.Il va alors se demander si il existe un empilement d'orange plus compact, plus dense ? En 1611, il fini par émettre une conjecture, c'est bien cet empilement qui est le plus dense/compact possible. \cite{empil4}
Deux siècles plus tard, en 1831, Gauss démontra que l'empilement de l'épicier était le plus dense des empilements réguliers. Une question se pose alors, et s'il existait un empilement irrégulier d'oranges plus dense que l'empilement régulier de l'épicier ?
C'est à la fin du $XX^{ième}$ siècle que le mathématicien Thomas Callister hales va démontrer que non, il n'existe pas d'empilement irrégulier plus dense que celui de l'empilement de l'épicier. \\
En 2016, Maryna Viasovska, une mathématicienne ukrainienne, va résoudre le problème des empilements compacts en dimension 8 et 24.\\\\
L'isopérimétrie, qu'est ce que c'est ?
En géométrie euclidienne, l'isopérimétrie est initialement l'étude des propriétés des formes géométriques du plan qui partagent le même périmètre, ce qui se généralise ensuite dans les autres espaces euclidiens. En physique et dans la nature, l'isopérimétrie se manifeste par un problème simple : pour un même volume, peut on chercher à minimiser la surface ? On comprends bien l'idée, avoir moins de surface, c'est avoir moins d'énergie à dépenser.\\\\
Qu'est ce que l'inégalité isopérimétrique ? En dimension deux, elle relie l'aire avec le périmètre, en dimension trois, elle relie le volume avec la surface..\\\\
Que va-t-il se passer si on essaye de mélanger les deux idées de l'empilement compact et de l'isopérimétrie ?
Nous allons nous intéresser de plus près à ces deux questions pour voir comment elles sont liées.
\cite{empil3}


  
\chapter{Empilements compacts}

Voyons dans un premier temps, l'empilement régulier de sphères égales en différentes dimensions.
Nous allons introduire les notions d'empilement, d'empilement régulier et irrégulier ainsi que de la densité, permettant d'aborder le reste de notre mémoire.
\cite{empil1}

On se place dans le cadre des espaces euclidiens de dimension finie.

\section{Découverte}
\begin{definition}[Empilement]
Un empilement est une manière de placer des objets dans l'espace sans que celles-ci ne se recouvrent, autrement dis, elles ne s'intersectent pas. 

On peut empiler différents objets, des disque, des ellipses par exemple.
\end{definition}

\begin{definition}[Empilement régulier]
Un empilement régulier est un empilement où le centre de chaque objet est placé sur un réseau $\Lamda = \sum_i \mathbb{Z} v_i$ avec $v_i$ un vecteur de cet espace euclidien.
\end{definition}

\begin{definition}[Empilement irrégulier]
Un empilement irrégulier est un empilement où les centres des objets ne sont pas placés sur un réseau.

On peut par exemple voir l'empilement d'Apollonius, qui est un empilement fractal, n'est pas régulier.
\end{definition}

\begin{definition}[Densité]
La densité d'un empilement est le rapport de l'espace occupé par cet empilement avec le volume de l'espace considéré.

Cela peut être par exemple un ensemble infini, en considérant une densité dans une sphère finie et en faisant tendre le rayon de cette sphère vers l'infinie, on aura alors une valeur finie comme densité.
\end{definition}

Dans toutes la suite on choisir d'empiler des objets égaux.

\section{Empilements de disques égaux en dimension 2}
\\\\
En 1 dimension, une boule est un segment, on cherche donc à empilement des segments de même longueur sur un axe.
On voit par un dessin que la densité est de 1, en effet les segments recouvrent complètement l'axe.\\\\
En 2 dimensions, la boule est le disque que l'on connaît. Nous travaillerons donc maintenant avec des disques de rayons r.
Nous voulons donc placer des disques dans le plan de la manière la plus dense possible.
Il y a deux arrangements qui se présentent à nous :
\begin{itemize}
    \item - l'arrangement rectangulaire
    \item - l'arrangement hexagonal
\end{itemize}

\begin{figure}[H]
~~~~~~~~~~~~~~~~~~~~
    \includegraphics[width = 4cm]{images/arrangement_rectangulaire.jpg}
    \hspace{2cm}
    \includegraphics[width = 4cm]{images/arrangement hexagonal.jpg}
    \caption{Arrangement rectangulaire et hexagonal}
    \label{fig:my_label}
\end{figure}
\newpage

{\noindent Quelle est la densité de ces deux empilements ?
Pour calculer la densité de ces deux empilements, nous allons devoir calculer la proportion d'aire des disques par rapport à l'aire du plan.
Pour cela, on va placer un point sur chaque centre de disque, et relier ces points par un segment si deux disques sont adjacents, on obtient alors les graphes suivants superposés aux empilements rectangulaire et hexagonal }

\begin{figure}[H]
~~~~~~~~~~~~~~~~~~~~
    \includegraphics[width = 4cm]{images/orange-1.png}
    \hspace{2cm}
    \includegraphics[width = 4cm]{images/orange-2.png}
    \caption{Arrangement rectangulaire et hexagonal}
    \label{fig:my_label}
\end{figure}

{\noindent Dans le cas de l'arrangement rectangulaire, on a pavé le plan par des carrés de côtés 2r, dont les sommets sont au centre des disques. Ainsi, chaque carré a pour aire $4r^2$. De cette aire, il y a une aire de $\pi r^2$ recouverte par des disques, en effet, on a quatre quart de disque de rayon r. Donc la densité de l'empilement est la densité de quatre quarts de disques de rayon r dans un carré de côté 2r qui est $\frac{\pi r^2}{4r^2} = \frac{\pi}{4}$.
Donc la densité de l'empilement rectangulaire est de $\frac{\pi}{4} \approx 0.785$.} \\\\
Dans le cas de l'arrangement hexagonal, on a pavé le plan par des triangles équilatéraux de côtés $2r$, dont les sommets sont aux centre des disques. Ainsi, chaque triangle a pour aire $\sqrt{3}r^2$. De cette aire, il y a une aire de $\frac{\pi r^2}{2}$ recouverte par des disques, en effet, on a 3 sixième de disque de rayon r. Donc la densité de l'empilement hexagonal est la densité de 3 sixième de cercle de rayon r dans un carré de côté 2r qui est $\frac{\pi r^2}{2 \sqrt(3) r^2} = \frac{\pi}{2 \sqrt(3)}$.
Donc la densité de l'empilement hexagonal est de $\frac{\pi}{2 \sqrt(3)} \approx 0.907.$

\subsection{Preuve de l'empilement le plus dense}
{\noindent On a vu que de vu que l'empilement le plus compact semblait être l'empilement hexagonal, il nous reste à le montrer.}\\\\
Si l'on compare des empilements réguliers, on verra que l'empilement hexagonal est bien le plus compact, qu'en est il des empilements irréguliers ?\\
Le mathématicien Alex Thue a eut l'idée de partionner l'espace en région astucieusement choisie  et de montrer que la densité est bien inférieure à la densité de l'empilement hexagonal dans chaque région.\\
Prenons des disques et agrandissons les tel qu'il remplissent l'espace du triangle.\\

\begin{figure}[H]
    \centering
    \includegraphics[width = 4cm]{images/orange-3.png}
    \label{fig:my_label}
\end{figure}

Le rayon des grands cercles nouvellement obtenu est égal au $\frac{2}{3}$ de la hauteur du triangle équilatéral, on trouve avec un petit calcul que ce rayon est $\frac{2 \sqrt{3}}{3}$.\\
Définissons maintenant les régions qui partionneront l'espace.

La première est la portion du plan qui n'est pas recouverte par des grands disques.

Pour les deux dernières régions, à chaque fois que des disques ont une intersection non vide, on les divises en secteur délimité par des rayons des cercles qui rejoignent les points d'intersection de leur frontière.
\begin{figure}[H]
    \hfill
    \includegraphics[width = 5cm]{images/orange-4.png}
    \hfill
    \includegraphics[width = 5cm]{images/orange-5.png}
    \hfill
    \hfill
    \label{fig:my_label}
\end{figure}
Le plan peut ainsi être partitionné avec ces 3 types de régions.\\

On voit une partition du plan avec cette image.
(image)
La \textbf{densité dans le premier type} est bien sûr nulle.\\

\textbf{Calcul de densité dans les régions de deuxième type.}\\
L'aire d'un secteur de disque de rayon r et d'ouverture $\theta$ est $\frac{\theta}{2} r^2$.\\
Donc la densité est $\frac{\frac{\theta}{2}}{r^2 \frac{theta}{2}} = \frac{1}{r^2} = \frac{3}{4} = 0.75$\\
Cette densité est bien inférieure à la densité de l'empilement hexagonal.\\

\textbf{Calcul de densité dans les régions de troisième type.}\\\\
Intéressons nous à la densité dans la troisième type de régions.\\
La région couverte par les petits disques est la réunion de deux secteurs de rayon 1 et d'ouverture $\theta$, qui a donc pour aire $\theta$.
L'aire de la région est la somme des aires des deux triangles isocèles ayant chacun les 2 côtés de longueur $r = \frac{2 \frac{\sqrt{3}}}{3}$ séparés par un angle $\theta$.
Par le théorème d'Al-KAshi, on a $c = \sqrt{2 r^2 (1-cos(\theta)}$. On cherche maintenant du côté manquant du double isocèle pour calculer son aire et on trouve $\sqrt{\frac{r^2(1+cos(\theta)}{2}}$. 
Son aire est donc le produit de ces deux longueurs qui donne $\frac{4}{3} sin(\theta)$.La densité des deux secteurs de disque dans ce double triangles isocèle est donc $\frac{3\theta}{4 sin(\theta)}$. On va étudier cette densité en fonction de $\theta$, déjà, $\theta \in [0, \frac{\pi}{3}]$.On va essayer de montrer que cette densité est bien inférieure à la densité de l'empilement hexagonal. En montrant que cette densité est croissante et que sa borne supérieure est bien inférieure à la densité de l'empilement hexagonal.

{\noindent Soit f : $\mathbb{R}\backslash \left\{  k\pi, k \in \mathbb{Z} \right\} \rightarrow \mathbb{R}$}\\

Pour trouver
\tabto{3cm} x $\longrightarrow$ $\frac{3x}{4sin(x)}$\\
f est prolongeable par continuité en 0, on a $\lim\limits_{\substack{x \rightarrow 0^+ \\ x \rightarrow 0^-}} f(x)$ f(x) = $\frac{3}{4}$, on pose donc f(0)=$\frac{3}{4}$\\
Montrons que f est croissante sur [0,$\frac{\pi}{3}$]. 

\begin{proof}
Pour cela, on va montrer que f'(x) est positive sur [0,$\frac{\pi}{3}$].\\
On a f'(x)=$\frac{3sin(x)}{4sin^2(x)}-\frac{3xcos(x)}{4sin^2(x)}$\\
On va étudier le signe de sin(x) - xcos(x) sur [0,$\frac{\pi}{3}$], elle vaut 0 en 0.\\
On dérive cette expression, on a cos(x) + xsin(x) - cos(x) = x sin(x). Et pour tout x $\in$ [0,$\frac{\pi}{3}$], on a x sin(x) $>$ 0, donc x $\rightarrow$ sin(x) -x cos(x) est croissante sur [0,$\frac{\pi}{3}$] donc elle est positive car elle vaut 0 en 0. Donc f'(x) $>$ 0 pour tout x $\in$ [0,$\frac{\pi}{3}$] et ainsi, f est croissante sur [0,$\frac{\pi}{3}$].
\end{proof}
{\noindent De plus}, on a $f(\frac{\pi}{3}) = \frac{\pi}{2 \sqrt{3}}$
{\noindent En dimension 3,} c'est le même type de raisonnement qu'a utilisé Thomas C. Halles pour démontrer la densité maximale, il a partitionné l'espace en régions, et a montré que, dans chaque type de régions, la densité est inférieure ou égale à la densité de l'empilement de l'épicier. Mais, car il y a un mais, le nombre de région à considérer est énorme, c'est pourquoi il a utilisé l'ordinateur pour calculer la densité dans tous les types de régions.

\section{Empilements de boules égales en dimension 3}
En 3 dimensions, les choses se compliquent, nous allons expliquer et donner des résultats sans énoncer de preuves.
On va s'intéresser à trois types d'empilement en 3 dimensions :
\begin{itemize}
    \item cubique
    \item triangulaire
    \item hexagonal
...
\begin{figure}[H]
    \includegraphics[width = 4cm]{images/arrangement_cubique.png}
    \hfill
    \includegraphics[width = 4cm]{images/arrangement_cubique_a_face_centree.png}
    \hfill
    \includegraphics[width = 4cm]{images/arrangemet_hexagonal.png}
    \caption{Arrangement cubique, triangulaire et hexagonal}
    \label{fig:my_label}
\end{figure}

On peut remarquer que l'empilement hexagonal peut se décliner sous plusieurs formes en donnant la même densité. Il n'y a donc pas d'unicité quant à la densité d'un empilement.

{\noindent Quelle est la densité de ces 3 empilements ?}\\
Pour calculer la densité de ces trois empilements, nous allons devoir calculer la proportion d'aire des boules par rapport au volume de l'espace.
Pour cela, on va de utiliser la même méthode que celle utilisée pour les disques : on va placer un point au centre de chaque boules, et relier ces points par un segment si deux boules sont adjacentes, on obtient alors les graphes suivants superposés aux empilements cubique, triangulaire et hexagonal : 
\cite{empil2}\\\\
Dans le cas de l'empilement cubique, on a pavé l'espace par des cubes de côtés $2r$, dont les sommets sont aux centre des boules. Ainsi, chaque cube a pour volume $(2r)^3 = 8r^3$. De ce volume, il y a un volume de $\frac{4}{3} \pi r^3$ occupé par des boules, en effet, on a 8 huitième de boule de rayon r. Donc la densité de l'empilement cubique est la densité d'une boule de rayon r dans un cube de côté 2r qui est \centerline{$\frac{\frac{4}{3} \pi r^3}{8r^3} = \frac{4}{24} \pi = \frac{\pi}{6}$}.\\
Donc la densité de l'empilement cubique est $\frac{\pi}{6} \approx 0.523$.\\
La densité de l'empilement hexagonal est $\frac{\pi}{\sqrt(18)} \approx 0.740$.\\
La densité de l'empilement triangulaire est $\frac{\pi}{\sqrt{27}} \approx 0.604$.\\

\section{Un mot sur les dimensions supérieures}
On remarque en augmentant de dimensions, que la densité globale a l'air de décroître.\\\\
Dorénavant, parlons des dimensions 8 et 24. Pourquoi passe-t-on de 3 dimensions à 8 et 24 dimensions ? \\
Car ces dimensions sont particulières, nous allons le voir plus en détail. Dans des articles publiés en 2016, la mathématicienne Ukrainienne Maryna Viazovska a résolu la question des empilements de sphères en 8 et 24 dimensions. Elle a démontrée que deux arrangements particuliers en dimension 8 et 24 permettent de placer les sphères de la façon la plus dense possible. Mais pourquoi ces dimensions particulières ? Quant on augmente en dimension, on augmente le nombre de cas à considérer, tout devient bien plus compliqué à étudier, pourtant il existe deux dimensions particulières dans lesquels les choses s'arrangent parfaitement : la dimension 8 et 24. Les réseaux des empilements sont respectivement nommé réseau E8 et réseau de Leech, chaque point du réseau représentant le centre d'une sphère. Plus l'on augmente en dimension, plus l'espace entre les oranges dans un empilement augmente, mais en dimension 8 et 24 il augmente suffisamment pour laisser la place à une nouvelle orange. Ce qui simplifie considérablement les objets à étudier.\\
L'empilement compact a amené à un autre problème, le "Kissing Number". On veut savoir combien de sphères identiques au maximum peuvent se ranger autour d'une sphère de même type. Le problème se pose dans toutes les dimensions, allant de la première à la 24ième.\\
\begin{figure}[H]
    \hspace{2cm}
    \includegraphics[width = 3.5cm]{images/Kissing-2d.png}
    \hspace{4cm}
    \includegraphics[width = 3.5cm]{images/Kissing-3d.png}
    \caption{Kissing Number en dimension 2 et 3}
    \label{fig:my_label}
\end{figure}

Combien de sphères identiques au maximum peuvent se ranger autour d'une sphère de même type ? Autrement dit combien peut on placer de sphères tangentes ne se coupant pas à une même sphère ?
Voici un tableau regroupant les valeurs connus ou estimées des valeurs possibles en chaque dimension.
On remarque pour les dimensions 2, 8 et 24 qu'il n'y a pas d'espaces entre les sphères tangentes, elles sont toutes tangentes avec les sphères voisines.
On a donc l'absence d'espace entre les boules qui permet l'unicité de l'arrangement le plus dense.

\begin{tabular}{|c|c|}
\rowcolor{orange!50}
\hline Dimension & Kissing Number\\
\hline 1 & 2 \\
\hline 2 & 6 \\
\hline 3 & 12 \\
\hline 4 & 24 \\
...
{\noindent Pour les autres dimensions, on peut se demander ce qu'il se passera pour la densité et,à en croire les différents résultats publiés, il semblerait que la densité décroît.}
%parler des codes d'erreurs 
Cependant il n'y a pas seulement l'empilement de cercles qui fascinent les scientifiques, mais aussi les empilements d'autres polygones réguliers comme le carré, le triangle,les hexagones  ou même les ellipses .
%automatiquement moins dense : moins bon rapport entre surface et volume ?
%conjecture kelvin,

\chapter{Les empilements compacts et l'inégalité isopérimétrique}
...
\newpage
{\noindent 1. La solution ne peut pas être une figure concave. En effet, en modifiant la figure comme dans l'image qui suit, on peut voir que l'on augmente l'aire sans changer le périmètre.}
\begin{figure}[H]
    \hfill
    \includegraphics[width = 4cm]{images/explication_0.png}
    \hfill
    \includegraphics[width = 4cm]{images/explication_1.png}
    \hfill
    \hfill
    \label{fig:my_label}
\end{figure}

{\noindent La réponse à ce problème est donc forcément une figure qui n'est pas concave, autrement dit une figure convexe.}\\\\
2. Elle doit avoir partout la même courbure.\\
En effet, regardons l'image suivante.
\begin{figure}[H]
    \hfill
    \includegraphics[width = 4cm]{images/explication_2.png}
    \hfill
    \includegraphics[width = 4cm]{images/explication_3.png}
    \hfill
    \hfill
    \label{fig:my_label}
\end{figure}
{\noindent Comment peut on trouver une surface plus grande que celle-ci ?}\\
...
{\noindent Nous allons aborder les théorèmes isopérimétrique, maintenant que nous avons expliqués qu'est ce que cela était intuitivement, nous allons essayer de mêler ces idées avec les mathématiques.}\\
...
\begin{definition}[Courbe de Jordan]
...
{\noindent L'idée derrière les séries de Fourier est de décomposer une fonction t-périodique est une somme, finie ou infinie, de fonctions sinusoïdales.}\\
On suppose désormais que f est $L^p$ intégrable au sens de Lebesgue.
\begin{definition}[Coefficient de Fourier de f]
...
\begin{definition}[Égalité de Parseval]
...
{\noindent Introduisons maintenant des inégalitées qui seront utiles, ici nous ne le démontreront pas car elle sont très compliquée.}
...
{\noindent Afin de comprendre le théorème isopérimétrique, nous allons introduire les concepts mathématiques nécessaires à sa compréhension}

\begin{definition}[La formule de Stokes]
...
\begin{proof}[Preuve du théorème isopérimétrique pour les courbes de Jordan]
{\noindent Pour prouver ce théorème, nous allons utiliser la théorie de la mesure de Lebesgue. D'après le théorème de Jordan, $\mathbb{C} \backslash \Gamma$ possède exactement deux composantes connexes, l'une notée A bornée et l'autre non bornée. On a alors S=$\lambda$(A).\\

{\noindent De plus, si on pose $f=x+iy$, avec f orientée positivement, on a avec la formule de Green-Riemann avec Q: (x,y) $\rightarrow$ $\frac{x}{2}$, P: (x,y) $\rightarrow$ $\frac{-y}{2}$ et $\Omega$ l'intérieur de la courbe $\Gamma$ on a $\frac{\partial Q}{\partial x}$ - $\frac{\partial P}{\partial y}$=1 et donc \\
...
{\noindent \textbf{}Nous allons maintenant énoncer d'autres théorèmes isopérimétriques, nous les donnons pour comprendre ce qu'il se passe en dimensions supérieure. Nous avons choisis de ne pas les démontrer.}
...
{\noindent En dimension supérieure à 3, nous devons donner plus de contexte car le théorème est plus délicat à mettre en place.}\\
...
\section{Conclusion : Du nid d'abeille aux bulles de savons}
...
\chapter{La conjecture du nid d'abeille}

\section{Introduction : les nids d'abeilles}
...
\section{Preuve de la conjecture}
...
\begin{definition}[Courbe lisse]
Une courbe dans le plan de classe $\mathcal{C}^{\infty}$ sera qualifiée de lisse.
\end{definition}

\begin{definition}[Graphe de $\mathbb{R}^2$ composé de courbes lisses]
...
\begin{definition}[Graphe localement fini]
...
\begin{definition}[Courbe rectifiable]
...
{\noindent Une courbe rectifiable est donc simplement une courbe dont on peut calculer sa longueur.}\\
...
{\noindent Voyons maintenant ce que cela donne en dimension finie.}
...
{\noindent Nous ne démontrerons pas que ce théorème implique le théorème du nid d'abeille pour les ensembles ouverts non connexes car la démonstration est très compliquée, mais pas très longue, elle utilise des théories mathématiques comme celle des courants.}\\
...
\subsection{La conjecture du nid d'abeille sur un tore}
{\noindent Ce théorème va nous permettre de passer de l'infini au fini.}
...
{\noindent Montrons que le théorème de la conjecture du nid d'abeille sur un tore implique la conjecture du nid d'abeille version finie.} \\\\
...
\begin{preuve}
{\noindent\underline{Preuve} : \textit{Le théorème de la conjecture du nid d'abeille sur un tore implique le théorème de la conjecture du nid d'abeille version finie}}\\\\
...
\begin{figure}[H]
    \hfill
    \includegraphics[width = 3.5cm]{images/graphe_0.png}
    \hfill
    \includegraphics[width = 3.5cm]{images/graphe_1.png}
    \hfill
    \includegraphics[width = 3.5cm]{images/graphe_2.png}
    \hfill
    \hfill
    \label{fig:my_label}
\end{figure}
...
\begin{figure}[H]
    \hfill
    \includegraphics[width = 3.5cm]{images/graphe_3.png}
    \hfill
    \includegraphics[width = 3.5cm]{images/graphe_4.png}
    \hfill
    \includegraphics[width = 3.5cm]{images/graphe_5.png}
    \hfill
    \hfill
    \caption{Cheminement du graphe planaire au graphe sur un tore}
    \label{fig:my_label}
\end{figure}
...
\subsection{L'inégalité isopérimétrique hexagonale pour les courbes planes fermées.}

{\noindent Maintenant, tout se passera sur un compact, plus précisément un tore.}\\
...
{\noindent Nous énonçons maintenant le théorème final, l'inégalité isopérimétrique hexagonale pour les courbes planes fermées. }Pour démontrer ce dernier théorème, Thomas C. Halles a trouvé différentes bornes inférieures de la longueur du graphe selon différents cas, qui utilisent différents théorèmes isopérimétriques comme le théorème isopérimétrique pour les polygones, qui énonce qu'à périmètre fixé, c'est le polygone régulier qui enferme la plus grande aire. Il a ensuite au cas par cas (il y en a beaucoup), calculé $\Delta (P)$ et montré qu'il était positif. Nous ne présenterons pas cette preuve qui est très longue, parfois compliqué et répétitive.\\
...
\begin{figure}[H]
    \hfill
    \includegraphics[width=5cm]{images/seingeur.png} 
    \hfill
    \label{fig:my_label}
\end{figure}
...
Dans cette partie, nous allons illustrer par des simulations les concepts précédemment vus d'empilements compacts et d'isopérimétrie.\\\\
Dans une première partie, nous allons, calculer la constante isopérimétrique de graphes donnés, illustrant ainsi les théorèmes énoncés dans le chapitre sur la conjecture du nid d'abeille. \\\\
Dans une dernière partie, nous allons, estimer la densité de certains empilements, illustrant ainsi les résultats obtenus dans le chapitre sur l'empilement compact.\\\\
Tous les algorithmes importants qui ont été utilisés pour faire ces simulations seront expliqués en détails, par des schémas et des explications. Le reste sera laissé en annexe.

\section{Première partie : Isopérimétrie}
...
\begin{figure}[H]
    ...
    \hfill
    \centering \includegraphics[width=5cm]{images/pic2.png} 
    \hfill
    \centering \includegraphics[width=5cm]{images/pic3.png}
    \hfill
    \caption{Déroulement de l'algorithme}
    \label{fig:my_label}
\end{figure}

{\noindent Pour appuyer nos résultats pratiques, nous avons calculé théoriquement la valeur exacte de la constante isopérimétrique des graphes illustrés dans les exemples suivants. Afin de voir que les valeurs trouvées pratiquement seront très proches des valeurs théoriques, donnant ainsi du crédit à notre algorithme. Elles seront mises en annexe dans la partie code}

\subsection{Résultats}


{\noindent Voici quelques résultats pour le triangle, le carré et l'hexagone.}

\begin{figure}[H]
    \hfill
    \includegraphics[width = 3.5cm]{images/qqq3.png}
    \hfill
    \hfill
    \caption{Constante isopérimétrique de l'empilement triangulaire = 2.27}
    \label{fig:my_label}
\end{figure}

\begin{figure}[H]
    \hfill
    \includegraphics[width = 3.5cm]{images/eee3.png}
    \hfill
    \hfill
    \caption{Constante isopérimétrique de l'empilement triangulaire = 2.00}
    \label{fig:my_label}
\end{figure}

\begin{figure}[H]
    \hfill
    \includegraphics[width = 3.5cm]{images/ss1.png}
    \hfill
    \hfill
    \caption{Constante isopérimétrique de l'empilement triangulaire = 1.87}
    \label{fig:my_label}
\end{figure}

Ces résultats correspondent bien aux valeurs trouvées théoriquement dans l'annexe.

\subsection{Remarque sur les altérations aléatoires compactes du graphe}
{\noindent Une altération aléatoire compacte d'un graphe consiste à déplacer chaque point du graphe dans une direction aléatoire et une distance aléatoire bornée par un réel fixé.}\\
...
{\noindent On voit donc que la constante isopérimétrique augmente. Ainsi la constante isopérimétrique est sensibles aux altérations aléatoires compactes.}

\section{Deuxième partie : Densité}
{\noindent Dans cette deuxième partie sur la densité, nous allons essayer de créer différentes situations, et estimer la densité moyenne dans chacune de ces situations.}

\subsection{Algorithme de calcul de la densité}
{\noindent L'algorithme de calcul de la densité consiste simplement à calculer la somme des aires des objets et de la diviser par l'aire dans laquelle ces objets sont contenus.}\\
...
\begin{figure}[H]
    \hfill
    \includegraphics[width = 6cm]{images/compact.png}
    \hfill
    \label{fig:my_label}
    \caption{Empilement compact aléatoire}
\end{figure}
...
{\noindent On voit que la densité décroît au fur et à mesure que le nombre de côtés augmente. On voit aussi que la densité n'excède jamais $0.5$, ce qui peut paraître vraiment très peu, il a été démontré qu'avec cet méthode, on peut espérer une densité maximale de $0.54$ environ.}

\subsubsection{Polygones réguliers dans un compact : une méthode moins aléatoire}

{\noindent Dans cette nouvelle simulation, nous avons gardés le principe de placer des objets aléatoirement en rajoutant une nouvelle condition : on a rajouté une gravité, les permettant de s'écraser au sol.}

\begin{figure}[H]
    \hfill
    \includegraphics[width = 6cm]{images/gravity_0.png}
    \hfill
    \includegraphics[width = 6cm]{images/gravity_1.png}
    \hfill
    \hfill
    \label{fig:my_label}
    \caption{Empilement compact aléatoire avec de la gravité}
\end{figure}

{\noindent La densité grimpe en flèche pour atteindre rapidement plus de 0.5, on en présentera donc pas les résultats de densités obtenus.}\\
...
\begin{figure}[H]
    \hfill
    \includegraphics[width = 6cm]{images/gravity_2.png}
    \hfill
    \label{fig:my_label}
    \caption{Empilement compact aléatoire}
\end{figure}
...
{\noindent Dans cette dernière partie de simulation, nous avons décidés de placer des polygones réguliers convexes à n côtés selon les empilements réguliers carré et hexagonal.}\\
...
\begin{figure}[H]
    \hfill
    \includegraphics[width = 5cm]{images/empilement_carre_12.png}
    \hfill
    \includegraphics[width = 5cm]{images/empilement_hexagonal_19.png}
    \hfill
    \hfill
    \label{fig:my_label}
    \caption{Empilement régulier de polygones réguliers convexes selon le réseau carré et hexagonal}
\end{figure}

{\noindent Voici un exemple d'empilement parfaitement adapté et l'autre très peu adapté.}

\begin{figure}[H]
    \hfill
    \includegraphics[width = 5cm]{images/parfait_1.png}
    \hfill
    \includegraphics[width = 5cm]{images/parfait_0.png}
    \hfill
    \hfill
    \label{fig:my_label}
    \caption{Empilement non adapté et adapté}
\end{figure}

\newpage

\section{Troisième partie : Diagramme de Voronoï}
...
\begin{figure}[H]
    \hfill
    \includegraphics[width = 3.5cm]{images/0.png}
    \hfill
    \includegraphics[width = 3.5cm]{images/50.png}
    \hfill
    \includegraphics[width = 3.5cm]{images/100.png}
    \hfill
    \hfill
    \label{fig:my_label}
\end{figure}
...
\begin{figure}[H]
    \hfill
    \includegraphics[width = 3.5cm]{images/150.png}
    \hfill
    \includegraphics[width = 3.5cm]{images/200.png}
    \hfill
    \includegraphics[width = 3.5cm]{images/350.png}
    \hfill
    \includegraphics[width = 3.5cm]{images/450.png}
    \hfill
    \hfill
    \caption{Étapes de l'algorithme}
    \label{fig:my_label}
\end{figure}
{\noindent
...
{\noindent Nous allons maintenant présenter les résultats des diagrammes obtenus, ils sont surtout intéressant visuellement. Ils permettent de conforter l'intuition que nous avons sur le sujet global des empilements compacts liés à l'aspect du pavage du plan. Pour illustrer l'empilement compact, nous allons voir l'exemple des hexagones. On peut voir les hexagones se tracer distinctement à partir de la troisième étape, et finir avec un empilement compact d'hexagones.} 
\begin{figure}[H]
    \hfill
    \includegraphics[width = 3.5cm]{images/hexa_0.png}
    \hfill
    \includegraphics[width = 3.5cm]{images/hexa_1.png}
    \hfill
    \includegraphics[width = 3.5cm]{images/hexa_2.png}
    \hfill
    \includegraphics[width = 3.5cm]{images/hexa_3.png}
    \hfill
    \hfill
    \caption{Diagramme de Voronoï des centres des disques de l'empilement hexagonal}
    \label{fig:my_label}
\end{figure}

{\noindent Il est cependant possible de paver l'espace avec d'autres formes géométriques grâce à cet algorithme. Par exemple, faire des altérations compacte du graphe, ou même simplement paver l'espace avec des carrés.}
\cite{Voronoi}

\addcontentsline{toc}{part}{Annexe}

\begin{appendices}
\chapter{Rappels sur les polygones}

{\noindent Cette annexe a pour but de fournir un rapide rappel sur les polygones de manière générale.}

\begin{definition}[Polygone]
Un polygone est une figure géométrique plane formée d'une ligne brisée fermée, c'est-à-dire d'une suite cyclique de segments consécutifs.
\end{definition}

\begin{definition}[Sommets d'un polygone]
Les extrémités des segments sont appelés les sommets du polygone.
\end{definition}

\begin{definition}[Côtés d'un polygone]
Les segments du polygone sont appelés les côtés d'un polygone.
\end{definition}

\begin{definition}[Polygone équilatéral]
Un polygone équilatéral est un polygone qui a tous ses côtés de même longueur.
\end{definition}

\begin{definition}[Polygone équiangle]
Un polygone équiangle est un polygone dont les angles internes sont égaux.
\end{definition}

\begin{definition}[Polygone régulier]
Un polygone régulier est un polygone à la fois équilatéral et équiangle.
\end{definition}

\begin{definition}[Polygone simple]
Un polygone est dit simple si deux côtés non consécutifs ne se rencontrent pas et deux côtés consécutifs n'ont en commun que l'un de leur sommets.
\end{definition}
...
\begin{definition}[Polygone concave]
Un polygone simple qui n'est pas convexe est dit concave.
\end{definition}

Il est utile de faire la nuance quand on parle de polygone régulier, ce qui mtoive l'introduction de ces deux définitions.

\begin{definition}[Polygone régulier étoilé]
Un polygone régulier étoilé est un polygone régulier non convexe.
\end{definition}

\begin{definition}[Polygone régulier convexe]
Un polygone régulier convexe est un polygone régulier convexe.
\end{definition}

\begin{tcolorbox}[colback=red!6]
\begin{proposition}[Angle interne d'un polygone régulier convexe]
Soit P un polygone régulier convexe à n côtés, A et B deux sommets consécutifs, alors AOB = $\frac{2 \pi}{n}$.
\end{proposition}
\end{tcolorbox}

\begin{definition}[Apothème]
L'apothème d'un polygone régulier convexe est le rayon du cercle inscrit dans ce polygone. C'est donc la longueur du segment joignant le centre du polygone au milieu d'un côté.
\end{definition}

\begin{definition}[Rayon d'un polygone régulier convexe]
Le rayon d'un polygone régulier convexe P est le rayon du cercle circonscrit à P et est noté $\rho$.
\end{definition}

\begin{tcolorbox}[colback=red!6]
\begin{proposition}[Périmètre d'un polygone régulier convexe]
Soit P un polygone régulier convexe à n côtés, $n \geq 3$, et soit a la longueur d'un côté, alors son périmètre est donné par $P = na$.
\end{proposition}
\end{tcolorbox}

\begin{tcolorbox}[colback=red!6]
\begin{proposition}[Périmètre d'un polygone régulier convexe]
Soit P un polygone régulier convexe à n côtés, et soit a la longueur d'un côté, alors son périmètre est donné par $P = 2n\sin(\frac{pi}{n}) \rho$.
\end{proposition}
\end{tcolorbox}

\begin{tcolorbox}[colback=red!6]
\begin{proposition}[Aire d'un polygone régulier convexe]
Soit P un polygone régulier convexe à n côtés, alors son aire est la somme des aires des n triangles isocèle de hauteur h (l(apothème) et de côtés a, on a donc : $S = n \times \frac{ah}{2} = \frac{Ph}{2}$.
\end{proposition}
\end{tcolorbox}

\begin{tcolorbox}[colback=red!6]
\begin{proposition}[Aire d'un polygone régulier convexe]
Soit P un polygone régulier convexe à n côtés et soit a la longueur d'un côté. Alors son aire est donnée par $S = \frac{na^2}{4 \tan(\frac{\pi}{n})} = \frac{n}{2} \sin(\frac{2 \pi}{n}) \rho ^2$
\end{proposition}
\end{tcolorbox}

\begin{tcolorbox}[colback=red!6]
\begin{proposition}
Si on suppose l'aire de ce polygone régulier convexe à n côtés égal à 1, on aura $L^2 = 4n tan(\frac{pi}{n}$, et on aura aussi $L = 2 \sqrt{n tan(\frac{\pi}{n})}$
\end{proposition}
\end{tcolorbox}

\textbf{Remarque :} 
Pour l'hexagone, on aura $L = 2 \sqrt[4]{12}$, qu'on notera $\pi _6$, d'une manière générale, on notera le périmètre d'un polygone régulier convexe à n côtés $\pi _n$.
\newpage


{\noindent Nous allons donner des informations générales sur l'aire, la taille d'un côté d'aire 1 et si il pave le plan, pour plusieurs figures géométriques.}\\\\

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\begin{tabular}{|c|c|c|c|}
\rowcolor{orange!50}
\hline Figure   & Aire  & Côté pour une aire de 1 & Pave le plan ? \\
\hline Hexagone régulier   & $\frac{3\sqrt{3}}{2} * c^2$ & $\sqrt{\frac{2}{3\sqrt{3}}}$ & oui\\
\hline Triangle équilatéral   &$\frac{\sqrt{3}}{4} * c^2$  & $\frac{2}{\sqrt[4]{3}}$ $\approx 1.5197$  & oui\\
\hline Carré  & $c^2$ & 4*c & oui \\
\hline
\end{tabular}

\begin{figure}[H]
    \hfill
    \includegraphics[width = 5cm]{images/hexagone_regulier.png}
    \hfill
    \includegraphics[width = 5cm]{images/triangle equilateral.png}
    \hfill
    \includegraphics[width = 5cm]{images/c1.png}
    \caption{Hexagone régulier ~~~~~~~~~~~~~~~~~~ Triangle équilatéral ~~~~~~~~~~~~~~~~ Carré}
    \hfill
    \label{fig:my_label}
\end{figure}
...
\begin{appendices}
\chapter{Code et aspect mathématique des simulations informatiques}
...
{\noindent \Large\textbf{Comment générer des polygones réguliers \\}}

{\noindent Pour faire nos simulations nous avons besoin de savoir créer efficacement des polygones réguliers.
...
\section{Théorie de la simulation de l'inégalité isopérimétrique}

\subsection{Simulation 1 : Pavage hexagonale \\}

{\noindent Le but de cette simulation sera d'estimer l'inégalité isopérimétrique pour le pavage hexagonale. Nous verrons que le théorème est bien vérifié : l'inégalité est bien atteinte !} \\

{\noindent \textbf{\Large Génération d'un hexagone régulier d'aire 1}}\\\\
{\noindent Le côté d'un hexagone d'aire 1 vaut $\sqrt{\frac{2}{3\sqrt{3}}}$ \\
On le note $c_h__1$ }\\
(centré en l'origine)
\begin{figure}[H]
    \hfill
    \includegraphics[width = 6cm]{images/hexagon_dots.png}
    \hfill
    \hfill
    \label{fig:my_label}
\end{figure}
...
\begin{figure}[H]
    \hfill
    \includegraphics[width = 3.5cm]{images/h1.png}
    \hfill
    \includegraphics[width = 3.5cm]{images/h2.png}
    \hfill
    \includegraphics[width = 3.5cm]{images/h3.png}
    \hfill
    \hfill
    \caption{Étape 1,2 et 3}
    \label{fig:my_label}
\end{figure}

\textbf{\Large Conjecture du nombres d'hexagones à chaque étapes \\}

{\noindent Soit la suite d'entiers naturels $(NH_n)_n_\in_\mathbb{N}$, qui conjecture le nombre total d'hexagones à l'étape n, définie par récurrence :} 
...
{\noindent \underline{Initialisation }} : Pour $n=1$:\\
On a $NH_1 = 1$ selon la conjecture de base, et selon la formule on a aussi $1$. 
Donc $P_1$ est vraie. \\

{\noindent \underline{Hérédité}} : On suppose que $P_n$ est vraie pour un certain $n \in \mathbb{N}^*$. \\

{\noindent On a $NH_n = n(n-1) \times 3+1$ }\\
...
{\noindent Soit la suite d'entiers naturels $(NT_n)_n_\in_\mathbb{N}$, qui conjecture le nombre total de traits à l'étape n, définie par récurrence :}
...
\subsection{Simulation 2 : Pavage triangulaire \\}

{\noindent Le but de cette deuxième simulation sera d'estimer l'inégalité isopérimétrique pour le triangle. On constatera qu'elle vérifie bien l'inégalité !}\\


{\noindent \textbf{\Large Génération d'un triangle équilatéral d'aire 1}}\\\\
Le côté d'un triangle équilatéral d'aire 1 vaut $\frac{2}{\sqrt[4]{3}}$.

(centré en l'origine)

\centerline{\includegraphics[width=6cm]{images/tttt.png}} \\


\begin{figure}[H]
    \hfill
    \includegraphics[width = 3.5cm]{images/t1.png}
    \hfill
    \includegraphics[width = 3.5cm]{images/t2.png}
    \hfill
    \includegraphics[width = 3.5cm]{images/t3.png}
    \hfill
    \hfill
    \caption{Étape 1, 2 et 3}
    \label{fig:my_label}
...
{\noindent \textbf{Conjecture du nombres de triangles à chaque étapes \\}}
...
{\noindent \underline{Initialisation}} : Pour n = 1 : $NTR_1 = 1$ pour les deux formules. Donc $P_1$ est vraie. \\
{\noindent \underline{Hérédité} : On suppose $P_n$ vraie pour un certain rang $n \in \mathbb{N}$. }\\
On a alors : $NTR_n = 4^{n-1}$ \\
Puis on a par la conjecture : $NTR_n_+_1 = 4^n$ \\
Donc $P_n_+_1$ est vraie donc $P_n$ est vraie $\forall n \in \mathbb{N}$ \\

{\noindent{ \textbf{\Large Conjecture du nombres de traits à chaque étapes }}}\\\\
...
{\noindent \textbf{\Large Calcul de l'inégalité isopérimétrique \\}}

{\noindent Le périmètre est donné par la multiplication du nombre de traits avec la longueur d'un trait, on aura donc comme périmètre à l'étape n : }\\
...
\begin{figure}[H]
    \hfill
    \includegraphics[width = 3.5cm]{images/c1.png}
    \hfill
    \includegraphics[width = 3.5cm]{images/c2.png}
    \hfill
    \includegraphics[width = 3.5cm]{images/c3.png}
    \hfill
    \hfill
    \caption{Étape 1,2 et 3}
    \label{fig:my_label}
...
\section{Les diagrammes de Voronoï}
...
%\cite{Voronoi}
{\noindent Nous avons précédemment vu le diagramme de Voronoï pour l'hexagone, nous allons présenter les résultats pour le carré, le carré dit perturbé et enfin pour le triangle.\\
...
\hfill
    \hfill
    \includegraphics[width = 3.5cm]{images/square_0.png}
    \hfill
    \includegraphics[width = 3.5cm]{images/square_1.png}
    \hfill
    \includegraphics[width = 3.5cm]{images/square_2.png}
    \hfill
    \hfill
...
\hfill
    \hfill
    \includegraphics[width = 3.5cm]{images/square_3.png}
    \hfill
    \includegraphics[width = 3.5cm]{images/square_4.png}
    \hfill
    \hfill
    \caption{Diagramme de Voronoï d'un ensemble de points placés selon les centres des disques de l'empilement carré.}
    \label{fig:my_label}
\end{figure}
...
\begin{figure}[H]
    \hfill
    \includegraphics[width = 3.5cm]{images/square_rand_0.png}
    \hfill
    \includegraphics[width = 3.5cm]{images/square_rand_1.png}
    \hfill
    \includegraphics[width = 3.5cm]{images/square_rand_2.png}
    \hfill
    \hfill
\end{figure}
\begin{figure}[H]
\hfill
    \hfill
    \includegraphics[width = 3.5cm]{images/square_rand_3.png}
    \hfill
    \includegraphics[width = 3.5cm]{images/square_rand_4.png}
    \hfill
    \hfill
    \caption{Diagramme de Voronoï d'un ensemble de points placés sur un grillage carré un peu perturbé.}
    \label{fig:my_label}
\end{figure}
{\noindent Voici un exemple d'altération aléatoire du graphe. Une altération aléatoire sur un graphe consiste à déplacer chaque point du graphe dans une direction aléatoire et une distance aléatoire bornée. Comme appliqué ci-dessus.} \\
...
\begin{figure}[H]
    \hfill
    \includegraphics[width = 3.5cm]{images/triangle_0.png}
    \hfill
    \includegraphics[width = 3.5cm]{images/triangle_1.png}
    \hfill
    \includegraphics[width = 3.5cm]{images/triangle_2.png}
    \hfill
    \hfill
\end{figure}
\begin{figure}[H]
\hfill
    \hfill
    \includegraphics[width = 3.5cm]{images/triangle_3.png}
    \hfill
    \includegraphics[width = 3.5cm]{images/triangle_4.png}
    \hfill
    \includegraphics[width = 3.5cm]{images/triangle_5.png}
    \hfill
    \hfill
    \caption{Diagramme de Voronoï d'un ensemble de points situés sur les sommets des hexagones d'un pavage hexagonale.}
    \label{fig:my_label}
\end{figure}
